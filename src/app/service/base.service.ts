...
export class BaseService {
  ...
  getAll(dataType: string): Observable<any> {
    ...
    this.http.get(url).forEach((data) => this.observables[dataType].next(data));

    return this.observables[dataType];
  }
  create(dataType: string, row: any): void {
    let url = `${this.serverUrl}${dataType}`;
    this.http.post(url, row).forEach((response) => this.getAll(dataType));
  }
  update(dataType: string, row: any): void {
    let url = `${this.serverUrl}${dataType}/${row.id}`;
    this.http.put(url, row).forEach((response) => this.getAll(dataType));
  }
  delete(dataType: string, row: any): void {
    let url = `${this.serverUrl}${dataType}/${row.id}`;
    this.http.delete(url).forEach((response) => this.getAll(dataType));
  }
}
//reaktív programozást tesz lehetővé,változik valami és értesít---micsoda??//
